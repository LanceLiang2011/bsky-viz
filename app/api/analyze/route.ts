...
const FIELD_PROMPTS = {
  en: {
    summary:
      "As a professional social media analyst, write a comprehensive personality profile (200-300 words) that provides deep psychological insights through analysis of this person's digital behavior. Focus on two distinct behavioral contexts:\n\n**Analyze {ORIGINAL POSTS}:** Examine their authentic self-expression, personal values, interests, and worldview as revealed through their original content. Look for patterns in their thoughts, passions, and unique perspectives.\n\n**Analyze {REPLIES TO OTHERS}:** Study their social interaction patterns, communication style, and interpersonal dynamics. Observe how they engage with others, their empathy levels, and relationship-building approaches.\n\n**Synthesize findings:** Create an integrated behavioral profile that captures their communication rhythm, emotional patterns, and distinctive personality traits across both contexts.\n\nAlways support your analysis with specific examples from their content. Create a rich, evidence-based psychological portrait that reveals their authentic digital personality.",
    animalSelection:
  ...
  "zh-cn": {
    ...
    animalExplanation:
      "写一段解释（建议100-120字），清楚地证明为什么你选择的特定动物{animalSelection}完美匹配此人的行为模式。将他们的实际发帖行为、沟通风格、社交互动和活动模式与该动物的自然特征和行为联系起来。参考导致这个选择的具体可观察行为 - 如他们的发帖频率、回应模式、对话领导风格或参与习惯。专注于来自其内容的具体行为证据，而非表面联想。创建一个令人信服的分析，证明此人与其精神动物之间清晰且合理的行为联系。重要：保证分析的动物和{animalSelection}所选择的动物一致。",
  },
};
