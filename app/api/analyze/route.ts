...
const FIELD_PROMPTS = {
  ...
  "zh-cn": {
    ...
    animalExplanation:
      "写一段详细的解释（建议100-120字），清楚地证明为什么你选择的特定动物完美匹配此人的行为模式。将他们的实际发帖行为、沟通风格、社交互动和活动模式与该动物的自然特征和行为联系起来。参考导致这个选择的具体可观察行为 - 如他们的发帖频率、回应模式、对话领导风格或参与习惯。专注于来自其内容的具体行为证据，而非表面联想。创建一个令人信服的分析，证明此人与其精神动物之间清晰且合理的行为联系。",
  },
};
